...
export default function Footer() {
    ...
    function toastWarn() {
        toast.warn("You can't subscribe, The form doesn't work properly now.")
    }
        ...
        <>
            <div className='w-[90%] mx-auto flex space-y-5 flex-col pb-5'>
                <div className='flex justify-between flex-col space-y-5 md:space-y-0 md:flex-row'>
                    <div>
                        <img src={`${process.env.PUBLIC_URL}/assets/logo-no-background.svg`} alt="logo" className="w-[60px] h-[60px] rounded-full drop-shadow-xl" />
                    ...
                    <form method='post' action='#' onSubmit={(e) => e.preventDefault()} className='flex md:items-center md:space-x-4 flex-col space-y-2 md:space-y-0 md:flex-row '>
                        ...
                        <div>
                            <button
                                type='submit'
                                className='text-sm w-full transition bg-custom-gradient px-3 py-2 rounded-2xl hover:scale-[1.02] hover:bg-opacity-85'
                                onClick={toastWarn}
                            >Subscribe</button>
                        </div>
                    </form>
